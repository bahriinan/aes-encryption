...
class AES {
        ...
        **/
        public function encrypt($x,$bsize) {
                $t = ""; // 16-byte block
        ...
        **/
        public function decrypt($y,$bsize) {
                $t = ""; // 16-byte block
        ...
        private function addRoundKey($round) {
                ...
                for ($i=0; $i<4; $i++) {
                        for ($j=0; $j<self::$Nb; $j++) {
								...
								//burada aha su altdaki sekiz
						
								if($this->bzz==8)
								{	$temp = $this->w[$round*self::$Nb+$j] >> (3-$i)*8;
								
								}else if($this==16)
									$temp = $this->w[$round*self::$Nb+$j] >> (3-$i)*16;
								else
									$temp = $this->w[$round*self::$Nb+$j] >> (3-$i)*32;
